...
int main(int argc, char* argv[])
{
    ...
    if (rank == 0) 
    {
        ...
        // // 每次取4个口令进行一组 SIMD 测试
        // for (int i = 0; i < 8; i += 4) {
        //     string batch_pws[4] = {test_pws[i], test_pws[i+1], test_pws[i+2], test_pws[i+3]};
        //     bit32 batch_states[4][4];

        //     // 调用你的并行函数
        //     MD5Hash_SIMD(batch_pws, batch_states);

        //     // 验证这一组中的 4 个结果是否均正确
        //     for (int j = 0; j < 4; j++) {
        //         stringstream ss;
        //         for (int k = 0; k < 4; k++) {
        //             ss << setw(8) << setfill('0') << hex << batch_states[j][k];
        //         }
        //         if (ss.str() != test_hashes[i + j]) {
        //             cout << "SIMD Test FAILED for: " << batch_pws[j] << endl;
        ...
        while (active_workers > 0)
        {
            ...
            {
                int batch_size = 0;
                int idx = 1; // 从索引 1 开始写入具体的任务数据

                // 核心改动：一个批次最多连续打包 50 个 PT 任务
                ...
                packet[0] = batch_size; // 写入本批次实际打包的任务数
        ...
        // Master 自身没有做哈希，传入 0。通过 MPI_Reduce 收集所有 Worker 中最大的哈希耗时，赋值给 time_hash
        ...
        cout << "Hash time:" << time_hash << "seconds"<<endl;//请不要µ修改这一行
