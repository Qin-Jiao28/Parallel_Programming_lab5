...
void MD5Hash_SIMD(string input[4], bit32 state[4][4])
{
	...
	for (int i = 0; i < n_blocks; i++)
	{
		...
		for (int j = 0; j < 16; j++)
		{
			...
			// 直接读取每个口令对应的 32-bit 字
            w0 = *(uint32_t*)(paddedMessage[0] + offset);
            w1 = *(uint32_t*)(paddedMessage[1] + offset);
            w2 = *(uint32_t*)(paddedMessage[2] + offset);
            w3 = *(uint32_t*)(paddedMessage[3] + offset);

			__m128i v = _mm_setr_epi32(w0, w1, w2, w3);
            x[j] = v;
		}
		__m128i a = A;
		__m128i b = B;
	...
	uint32_t resA[4], resB[4], resC[4], resD[4];
    _mm_storeu_si128((__m128i*)resA, A);
    _mm_storeu_si128((__m128i*)resB, B);
    _mm_storeu_si128((__m128i*)resC, C);
    _mm_storeu_si128((__m128i*)resD, D);

	for (int i = 0; i < 4; i++) {
        // 每个口令 i 的 state 分别从 resA[i], resB[i]... 中获取
        uint32_t raw_states[4] = { resA[i], resB[i], resC[i], resD[i] };
        for (int j = 0; j < 4; j++) {
            uint32_t value = raw_states[j];
            // 保持原有的大端序转换逻辑 [cite: 179]
            state[i][j] = ((value & 0xff) << 24) | ((value & 0xff00) << 8) |
